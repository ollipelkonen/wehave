...
float sdPlug( vec3 p, vec2 h )
{
  vec3 b = abs(p);
  float ta = atan2(b.x,b.y);
  float py = h.x;
  //h.x *= ta;
  
//  float kg =  max( max(length(p.x)-h.x, length(p.y)-py), b.z-h.y );
  float kg =  max( length(p.xy)-h.x, b.z-h.y );
  //kg =  max( ta-h.x, b.z-h.y );
  float mm = max( kg, 0 );
  
  //pMod1(mm,33);
  return sminPoly_MediaMolecule(mm,mm,0.1);
  return mm;
}

float jalkaScene( vec3 samplePoint ) {
  float t1 = fmod(fGlobalTime / 1.0, 1);
  float t2 = fmod(fGlobalTime / 1.0 + 0.25, 1);
  vec3 p1 = vec3(0,0,-3);
  Hit h;
  float g = min(
...
//  g = min(  sdPlug( samplePoint, vec2(1.5,0.5) ), g );
  return sdPlug(samplePoint, vec2(2.0,0.5) );
  g = min( sdPlug(samplePoint, vec2(2.0,0.5)), g );
  return g;
...
float carhead( vec3 samplePoint ) {
  ...
  float t1 = fmod(fGlobalTime / 1.0, 1);
  float t1t1 = pow( 1+EPSILON+cos(t1*4*PI), 2) / 4;
  samplePoint.y -= t1t1;
...
//  float jaw = sdBox( samplePoint - vec3(0.4,-0.6,0), vec3(0.5,0.5,0.4) );
  ...
  k = opSubtraction( sdSphere(samplePoint-vec3(4.5,-4.0,0),1.5), k );
...
k = fOpUnionSoft( jalkaScene(samplePoint+ vec3(0,4,0)), k, 0.6 );
  return k;
}
...
vec4 h =
    texture( texFFTSmoothed, 0.0 ) +
    texture( texFFTSmoothed, 0.1 ) +
    texture( texFFTSmoothed, 0.2 ) +
    texture( texFFTSmoothed, 0.3 ) +
    texture( texFFTSmoothed, 0.4 ) +
    texture( texFFTSmoothed, 0.5 ) +
    texture( texFFTSmoothed, 0.6 ) +
    texture( texFFTSmoothed, 0.7 ) +
    texture( texFFTSmoothed, 0.8 ) +
    texture( texFFTSmoothed, 0.9 ) +
    texture( texFFTSmoothed, 1.0 );
pos.x += (h.x+h.y+h.z)*10;
...
/*  eye.xy = uv.xy*5;
  ...
  else if ( fMidiKnob4 < 1.0 )
  {
    ...
    vec3 diffuseColor = vec3( 0.4, 0.5, 0.3 );
    ...
    float vor = voronoi( hit.normal.xy*5 );
    float vor2 = voronoi( hit.normal.yz*5 );
    vor *= 12/distance;
    vor2 *= 12/distance;
    ambientColor.x += vor / 20;
    ambientColor.y += vor2 / 20;
    diffuseColor.x += vor / 2;
    diffuseColor.y += vor2 / 2;
